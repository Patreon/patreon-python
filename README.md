...
Instead, it returns a [JSON:API resource object](https://github.com/Patreon/cartographer/blob/master/cartographer/parsers/jsonapi_parser.py#L6),
...
* `response.data().related_resource('campaign').attribute('pledge_sum')` to get the `pledge_sum` attribute from the `campaign` resource related to the main response data
